...
public class Player : MonoBehaviour
{
    ...
    public float[] CalculateHitForces()
    {
        ...
        /* float upForce = (_movementJoystick.Vertical >= _upperHitJoystickBound)
             ? _directHitUpMultiplier * _forceIndicator.fillAmount * _movementJoystick.Vertical * _directHitForceMultiplier
             : _forceIndicator.fillAmount * _directHitUpMultiplier;

         float forwardForce = (_movementJoystick.Vertical >= _upperHitJoystickBound)
             ? _forceIndicator.fillAmount * _directHitUpMultiplier * _directHitForceMultiplier
             : _forceIndicator.fillAmount * _directHitUpMultiplier;*/

        float upForce = (_movementJoystick.Vertical >= _upperHitJoystickBound)
            ? 8 * _forceIndicator.fillAmount
            : 1.5f;

        float forwardForce = (_movementJoystick.Vertical >= _upperHitJoystickBound)
            ? 8 * _forceIndicator.fillAmount
            : _directHitForceMultiplier * _forceIndicator.fillAmount;
